...
try:
    ...
    MERCHANT_WALLET = PublicKey(os.getenv("MERCHANT_WALLET"))
    CUSTOMER_WALLET_PRIVATE_KEY = os.getenv("CUSTOMER_WALLET_PRIVATE_KEY")
    SOLANA_API_URL = os.getenv("SOLANA_API_URL", "https://api.mainnet-beta.solana.com")
    GOOGLE_API_KEY = os.getenv("GOOGLE_API_KEY")
    
    if not all([MERCHANT_WALLET, CUSTOMER_WALLET_PRIVATE_KEY, SOLANA_API_URL, GOOGLE_API_KEY]):
        st.error(f"Missing configuration: {str(e)}")
        st.error("Please check your .streamlit/secrets.toml file, Streamlit Cloud secrets configuration, or environment variables.")
        st.stop()
